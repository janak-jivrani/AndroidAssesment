...
import com.zw.zwbase.data.response.User
...
object PreferenceUtil {
    ...
    private fun setSharedPreference(p_context: Context?) {
        if (!::m_sharedPreference.isInitialized && p_context != null) m_sharedPreference = p_context.getSharedPreferences(SHARED_PREF_NAME, Context.MODE_PRIVATE)
    }
    ...
    fun setUserDetails(p_context: Context?, user: User) {
        val gson = Gson()
        val json = gson.toJson(user)
        setStringSharedPref(p_context, Constant.USER_DETAILS, json)
    }

    fun getUserDetails(p_context: Context): User? {
        val gson = Gson()
        val json = getStringSharedPref(p_context, Constant.USER_DETAILS, null)
